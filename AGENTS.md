...
- **日期处理**：date-fns v4
- **图标**：lucide-react
- **文件存储**：`lib/storage.ts` 存储驱动抽象层（`VercelBlobDriver` 走 `@vercel/blob` / `LocalDiskDriver` 走本地磁盘，由 `STORAGE_DRIVER` 切换），附件上传
...
- **密码保护**：`proxy.ts`（Next.js 16 Middleware，matcher 匹配全部路由）+ `/login` 页 + `/api/auth` 接口 + 路由内 `lib/auth-guard.ts` 防御纵深
  - 认证方式：`app_auth` cookie（30 天）/ `Authorization: Bearer` 头
  - **cookie 内容**：`app_auth` 存的是由 `APP_PASSWORD` 派生的 HMAC token（`lib/auth-token.ts`），**不是明文密码**；中间件 `proxy.ts` 与路由内 `isAuthorized()` 均用 `verifyToken()` 校验，密码比对走常量时间比较。改密码后旧 token 自动失效。
  - **防御纵深**：所有变更型 API（POST/PATCH/DELETE）及导出/备份接口在路由内部再用 `isAuthorized()` 复核 token，即使中间件被绕过也不会未授权写入。只读 GET 列表接口仍由中间件统一把关。
  - **登录限流**：`/api/auth` 按客户端 IP 做内存固定窗口限流（5 分钟内最多 10 次失败，超出返回 429），成功登录清除惩罚。注意：内存限流为单实例级别，Serverless 多实例下不跨实例共享（个人应用可接受）。
  - 公开路径免认证：`/login`, `/api/auth`, `/manifest.json`, `/icons/`, `/uploads/`
  - 未认证 API 返回 401 JSON；未认证页面重定向到 `/login?from=<original_path>`
  - 若 `APP_PASSWORD` 未设置，则自动跳过认证（中间件与 `isAuthorized()` 均直接放行）
...
```bash
npm test          # vitest 单元测试（12 文件，172 测试）
