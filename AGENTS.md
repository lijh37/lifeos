...
app/                  # Next.js App Router 页面和 API
  ...
  ├── api/notes/[id]/ # 单条笔记操作
  ├── api/notes/batch/ # 批量操作（删除/归档/加标签）
  ├── api/budgets/    # 预算 CRUD
...
components/           # React 组件
  ...
  ├── chat.tsx        # AI 对话组件（核心）
  ├── note-list.tsx   # 笔记/任务列表（批量操作+拖拽排序+富文本编辑）
  ├── rich-editor.tsx # 富文本编辑器（基于 TipTap，输出自动 XSS 净化）
  ├── sidebar.tsx     # 导航（PC 侧栏 + 手机底部栏，10 项）
  ├── fab-button.tsx  # 悬浮快捷按钮（可拖拽）
  ├── export-button.tsx # 导出按钮（MD/JSON/CSV）
  ├── auto-backup.tsx # 自动备份钩子 + 管理面板
  ├── error-boundary.tsx # React Error Boundary（笔记/任务/习惯页面）
...
lib/                  # 核心逻辑
  ├── db.ts           # 数据库操作（5 表，搜索/标签/统计/清除/习惯趋势）
...
data/
  └── schema.sql      # 完整 DDL（5 表，含 conversations）
