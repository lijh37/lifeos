...
### 笔记 UI 优化 (2026-07-06)
- Toast 通知系统：安装 `sonner` + 全局 Toaster，pin/delete/batch/create/保存失败均有 toast 反馈
...
- 标签操作回滚：添加/移除标签失败时回滚 UI 和数据（之前静默忽略）
...
- 无限滚动修复：onScroll → IntersectionObserver（Base UI ScrollArea 自定义滚动条导致 onScroll 不可靠，滚动加载从未生效）
...
- 搜索结果不支持游标分页（仅首次查询）
- Cmd+K 命令面板及全局搜索 API 已移除（2026-07-06）
- 笔记列表无限滚动改用 IntersectionObserver（sentinel 元素 + rootMargin 400px），替代 onScroll 方案（Base UI ScrollArea 自定义滚动条导致 scroll 事件不可靠）
...
3. 标签批量整理
4. 附件上传前端集成（DB 层已完成）
5. 滚动加载改用 IntersectionObserver（需处理 observe 即时触发问题）
6. 移动端搜索框可改为点击展开模式（参考 iOS 备忘录）
